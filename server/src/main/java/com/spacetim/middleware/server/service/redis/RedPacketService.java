...
import com.spacetim.middleware.server.utils.RedPacketUtil;
import com.spacetim.middleware.server.utils.SnowFlake;
...
public class RedPacketService implements IRedPacketService {
    ...
    private static BigDecimal count = new BigDecimal(0d);
    @Autowired
        ...
        if (res){
            ...
            if (value!=null){
               ...
               count.add(result);
                ...
                log.info("当前用户抢到红包了：userId={} key={} 金额={} 累计金额={}",userId,redId,result,count);
                return result;
            }
